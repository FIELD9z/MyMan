...
function App() {
  ...
  function handleTagManagerToggle() {
  ...
  async function handleTagsChanged() {
    ...
    await refresh(activeType, query, nextTag, searchMode, false)
  }
  ...
  const workspaceEyebrow = showTagManager
    ? '信息组织'
    : showArchived
      ? '归档管理'
      : '今日视图雏形'
  const workspaceTitle = showTagManager
    ? '标签管理'
    : showArchived
      ? `${activeTypeLabel}归档`
      : `${activeTypeLabel}工作台`
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="view-nav" aria-label="内容视图">
          <button
            className={!showArchived && !showTagManager ? 'active' : ''}
            onClick={() => void handleArchivedToggle(false)}
          >
            当前内容
          </button>
          <button
            className={showArchived ? 'active' : ''}
            onClick={() => void handleArchivedToggle(true)}
          >
            归档箱
          </button>
          <button className={showTagManager ? 'active' : ''} onClick={handleTagManagerToggle}>
            标签管理
          </button>
        </nav>
        ...
        {!showArchived && !showTagManager && allTags.length > 0 ? (
      ...
      <section className="workspace">
        ...
        {showTagManager ? (
          <TagManager onChanged={handleTagsChanged} />
        ) : (
